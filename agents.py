    ...
    def process_rfp(self, rfp_id, filepath):
        ...
        try:
            ...
            if not questions:
                ...
                self.emit("error", message="No requirements found in the uploaded file.")
                return
    ...
    def process_document(self, rfp_id: int, doc_id: int, filepath: str) -> None:
        ...
        try:
            ...
            if not questions:
                ...
                self.emit("error", message=f"No requirements found in {os.path.basename(filepath)}")
                return
...
class DemoPrepAgent:
    ...
    def generate(self, rfp_id, customer_format=None):
        ...
        resp = self.client.messages.create(
            model=_MODEL,
            max_tokens=8000,
            system="You are a senior Okta Solutions Engineer. Respond with valid compact JSON only — no markdown fences, no prose.",
            messages=[{
                "role": "user",
                "content": f"""Create a demo plan for this RFP. Return ONLY compact JSON, no explanation.

Customer: {customer} | Industry: {industry}
Scope: {scope}

Requirements summary ({len(answered)} answered, {len(flagged)} flagged):
{json.dumps(q_digest[:30], separators=(',', ':'))}
{format_instruction}
{flagged_note}

JSON schema (be concise — max 20 words per string value):
{{"executive_summary":"2 sentences","total_minutes":60,"sections":[{{"order":1,"title":"string","okta_products":["LCM"],"priority":"critical|high|medium","estimated_minutes":10,"requirement_ids":[1],"demo_scenario":"1 sentence","demo_steps":["Step 1","Step 2","Step 3"],"talking_points":["Point 1","Point 2"],"differentiators":["1 differentiator"]}}],"questions_to_address":["item"],"recommended_demo_env":"string"}}

Rules:
- 4-6 sections max, ordered critical-first
- Group related requirements into logical demo flows
- demo_steps: 3-4 concrete sequential steps per section
- talking_points: 2-3 punchy messages per section
- Keep ALL string values under 25 words"""
            }],
        )
        ...
        if resp.stop_reason == "max_tokens":
            self.emit("error", "Response was truncated — too many requirements. Try processing a smaller RFP first, or the agent will retry with a shorter prompt.")
            return None

        data = _extract_json(raw) or {}
        if not data.get("sections"):
            preview = raw[:200].replace('\n', ' ')
            self.emit("error", f"Could not parse demo plan JSON. Model returned: {preview}")
            return None
...
def _sample_file_text(filepath, max_chars=3000):
    """Extract a text sample from a CSV or XLSX for customer detection."""
    ext = filepath.rsplit(".", 1)[-1].lower()
    chunks = []
