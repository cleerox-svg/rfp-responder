...
| Agent | File | Purpose |
|---|---|---|
| Product Owner | `.claude/agents/strategic/product-owner.md` | Owns user stories and scope; arbitrates what "done" looks like |
| Architect | `.claude/agents/strategic/architect.md` | Owns technical shape, API contracts, synthesizes assessment reports |
| Project Manager | `.claude/agents/strategic/project-manager.md` | Sequences work, identifies parallel waves, assigns agents |

### Tier 2 — Implementation

| Agent | File | Purpose |
|---|---|---|
| Backend Engineer | `.claude/agents/implementation/backend-engineer.md` | Flask routes, SSE, export handler, auth stubs |
| Frontend Engineer | `.claude/agents/implementation/frontend-engineer.md` | SPA, all pages, agent card UI, settings |
| Data Engineer | `.claude/agents/implementation/data-engineer.md` | SQLite schema, FTS5, indexes, thread-safety |
| AI Pipeline Engineer | `.claude/agents/implementation/ai-pipeline-engineer.md` | All 9 agent classes, tools, model constants, agentic loop |
| Integration Specialist | `.claude/agents/implementation/integration-specialist.md` | Okta OIDC, LiteLLM proxy, MCP connections, .env bootstrap |

### First task sequence
Spawn the 5 implementation agents in parallel for the assessment wave. Collect reports, pass to Architect for synthesis, then PM for sequencing. See `docs/multiAgentDesign.md` for the full interface design.
