...
> Deploy NaughtRFP to a single AWS EC2 instance for demo use.
...
Internet ──► EC2 Security Group (port 80)
                  │
               Nginx :80  (nginx:alpine container)
...
The script installs Docker, Docker Compose, creates `.env`, builds the containers, and starts everything. It will pause to let you edit `.env`.
...
Navigate to `http://YOUR_EC2_IP` in your browser (takes ~20-30s on first boot while Gunicorn initialises).
...
If you have a domain name pointed at your EC2 (A record → Public IP):
...
Certbot auto-configures Nginx for port 443 and sets up auto-renewal.
...
**Tip:** Stop the instance when not demoing. EBS data persists. Cost drops to ~$1.60/month (storage only).
